...
//sampleNews = `As soft deve, the`; //test meqsedi ile yaradilib
...
function uppercaseLetter(object) {
  ...
  for (let i = 0; i < object.length; i++) {
    if (
      ...
      object[i] !== ":" &&
      object[i] !== "-" &&
      object[i] * 0 !== 0 //reqem oldugunu yoxlamaq ucun
    ) {
      upperCaseLetterArr.push(object[i]);
    }
  }

  console.log(upperCaseLetterArr);
}

uppercaseLetter(sampleNews);
